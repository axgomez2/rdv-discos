...
<div class="bg-yellow-400-100 pb-10 overflow-hidden"
    ...
    <div class="relative">
        ...
        <div class="absolute top-4 left-4 z-20">
            <span class="bg-yellow-500 text-black px-3 py-1 rounded-md text-sm font-medium">
                ULTIMOS DISCOS
            </span>
        </div>
                ...
                @foreach($vinyls as $vinyl)
                    <div class="w-full flex-shrink-0">
                        <a href="{{ route('site.vinyl.show', ['artistSlug' => $vinyl->artists->first()->slug, 'titleSlug' => $vinyl->slug]) }}"
                           class="block relative h-[350px] group">
                            ...
                            <div class="relative z-10 h-full p-6 flex items-center">
                                ...
                                <div class="ml-6 flex flex-col justify-center text-white">
                                    <h3 class="text-2xl font-bold group-hover:text-yellow-500 transition-colors duration-300">
                                    ...
                                    <div class="flex gap-2 mt-3">
                                        @foreach($vinyl->genres as $genre)
                                            <span class="text-xs font-medium px-2.5 py-0.5 rounded-full bg-pink-500 text-white">
                                                {{ $genre->name }}
                                            </span>
                                        @endforeach
                                    </div>
                            ...
                            <div class="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <button type="button" class="text-black bg-yellow-500 hover:bg-yellow-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2">
                                    Ver Detalhes
                                </button>
                            </div>
                        </a>
